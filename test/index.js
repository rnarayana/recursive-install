...
describe("recursive install", function () {
  ...
  describe("test with option --skip-root", function () {
    ...
    it("installs packages, but not in root directory", function () {
      assert(
        ...
        "Should not install dependencies for root directory " +
          cwd +
          ". Directory Listing: " +
          fs.readdirSync(cwd)
      );
  ...
  describe("test with fail", function () {
    it("fails to install packages", function (done) {
      ...
      fs.ensureDirSync(cwd);
      fs.copySync(
        path.join(__dirname, "test-package-fail.json"),
      ...
      try {
        ...
        assert(err.status === 1, "Exist status should be 1, not " + err.status);
        done();
      }
    });
  });
